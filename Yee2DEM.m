...
for i = 1:nSteps
    ...
    for k = 1:Reg.n
        ...
        if bc{k}.NumS > 0
            for i=1:bc{1}.NumS
                Einc = bc{k}.s(i).fct(yC{k},i,t,bc{k}.s(i).paras);
                Eincp = bc{k}.s(i).fct(yC{k},i,t-ds/c_c/2,bc{k}.s(i).paras);

                Hy{k}(bc{k}.s(i).xpos,:) = Hy{k}(bc{k}.s(i).xpos,:) + dt/ds/c_mu_0*Einc;
                Ez{k}(bc{k}.s(i).xpos-1,:) = Ez{k}(bc{k}.s(i).xpos-1,:) - dt/ds/c_eps_0/c_eta_0*Eincp;
            end 
        end
